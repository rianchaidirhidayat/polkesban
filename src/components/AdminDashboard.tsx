            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {onForceSyncCloud && (
                <button
                  ...
                  className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-emerald-50 hover:bg-emerald-100 text-emerald-800 text-xs font-bold border border-emerald-300 shadow-2xs"
                >
                  <RefreshCw className={`w-3 h-3 text-emerald-700 ${isForceSyncing ? 'animate-spin' : ''}`} />
                  <span className="text-[11px]">{isForceSyncing ? 'Sinkron...' : 'Sync HP'}</span>
                </button>
              )}
